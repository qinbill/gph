...
int main(int argc, char* argv[])
{
  ...
  for (int err = 0; err < BLOCKMAXERR; err+=4) {
    ...
    for (int i = 0; i < queryN; i++) {
      ...
      int64_t truecount = 0;
      int q = i;
      for (int64_t d=0; d<indata->mNumData; d++) {
	// printf("process doc %lld of %lld\n", i, N);
	split(chunks, indata->mData + d*indata->mDataBytes, M, mplus, b);
	int blockerr = 0;
	for (int k=0; k<M; k++) {
	  int err = match((UINT8*)(chunks+k), (UINT8*)(querychunks+q*M+k), 8);
	  blockerr += err;
    ...
    //double trueErrate = (totalTruecount > 0)?(myabs(totalTruecount-totalGrountCost)*1.0/totalTruecount):0;
    fprintf(stderr, "TrueCount for %d: Sample: %d Real: %d Truecount: %lld\n", err, dataN, indata->mNumData, totalTruecount);
  }
